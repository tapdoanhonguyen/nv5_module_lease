...
if(defined('NV_IS_USER')){
	...
	if ($nv_Request->isset_request('delete_cid', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
		...
		if ($cid > 0 and $delete_checkss == md5($cid . NV_CACHE_PREFIX . $client_info['session_id'])) {
			...
			$nv_Cache->delMod($module_name);
			nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Customer', 'ID: ' . $cid, $admin_info['userid']);
			...
			try {
				...
				if ($exc) {
					...
					if (empty($row['cid'])) {
						...
						nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Customer', 'ID: ' . $row['cid'], $admin_info['userid']);
					}
					nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
				}
	...
	$xtpl = new XTemplate('customer.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
