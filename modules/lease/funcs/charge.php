...
if(defined('NV_IS_USER')){
	...
	if ($nv_Request->isset_request('change_status', 'post, get')) {
		...
		$query = 'SELECT active FROM ' . NV_PREFIXLANG . '_' . $module_data . '_charge WHERE cid=' . $cid;
		$row = $db->query($query)->fetch();
		if (isset($row['active']))     {
			$active = ($row['active']) ? 0 : 1;
			$query = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_charge SET active=' . intval($active) . ' WHERE cid=' . $cid;
			$db->query($query);
			$content = 'OK_' . $cid;
		}
	...
	if ($nv_Request->isset_request('delete_cid', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
		...
		if ($cid > 0 and $delete_checkss == md5($cid . NV_CACHE_PREFIX . $client_info['session_id'])) {
			...
			$nv_Cache->delMod($module_name);
			nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Charge', 'ID: ' . $cid, $admin_info['userid']);
	...
	if ($nv_Request->isset_request('submit', 'post')) {
		...
		if (empty($error)) {
			try {
				if (empty($row['cid'])) {
					...
					$stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_charge (title, active, adminid, crtd_date, userid_edit, update_date) VALUES (:title, :active, :adminid, :crtd_date, :userid_edit, :update_date)');
					...
					$weight = $db->query('SELECT max(update_date) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_charge')->fetchColumn();
					$weight = intval($weight) + 1;
					$stmt->bindParam(':update_date', $weight, PDO::PARAM_INT);


				} else {
					$stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_charge SET title = :title WHERE cid=' . $row['cid']);
				}
				...
				if ($exc) {
					$nv_Cache->delMod($module_name);
					if (empty($row['cid'])) {
						nv_insert_logs(NV_LANG_DATA, $module_name, 'Add Charge', ' ', $admin_info['userid']);
					} else {
						nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Charge', 'ID: ' . $row['cid'], $admin_info['userid']);
					}
					nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
				}
		...
		$row = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_charge WHERE cid=' . $row['cid'])->fetch();
		if (empty($row)) {
			nv_redirect_location(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
		}
	} else {
	...
	if (!$nv_Request->isset_request('id', 'post,get')) {
		...
		$db->sqlreset()
			->select('COUNT(*)')
			->from('' . NV_PREFIXLANG . '_' . $module_data . '_charge');
	...
	$xtpl = new XTemplate('charge.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
