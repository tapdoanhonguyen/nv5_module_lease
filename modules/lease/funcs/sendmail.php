		...
		if ($nv_Request->isset_request('submit', 'post')) {
			...
			if (empty($error)) {
				try {
					...
					$where ="se.yearmonth='". $row['yearmonth']  ."' AND se.cid=" . $row['cid'] ." AND se.pid=" . $row['pid'];
				
					$db->sqlreset()
						->select('se.id,  SUM(totalvnd) totalvnd ,SUM(totalusd) totalusd,SUM(amount) amount, se.pricevnd,se.priceusd,s.title_vi title,s.unitid,s.sid')
						->from('' . NV_PREFIXLANG . '_' . $module_data . '_service_extra se')
						->join(' LEFT JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_service s ON se.sid = s.sid ' )
						->group('se.sid')
						->where($where);
						print_r($db->sql());
					$data_service_extra = $db->query($db->sql());
					if ($exc) {
						$nv_Cache->delMod($module_name);
						$value = $data_contract;
						$weightse = 1;
						if(!empty($value)){
							$value['stitle'] = $array_sid_lease[$value['sid']]['title'];
							if (empty($row['id'])) {
						...
						if($data_service_extra->rowCount()>0){
							while( $value = $data_service_extra->fetch()  ){
								$weightse++;
								print_r($value);
								if($value['id']>0){
									if (empty($row['id'])) {
										$value['service_main'] = 0;
										$value['note'] = '';
										$stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_debitnote_extra (debitnoteid, pid, cid, yearmonth, adddate, serviceid, service_name, service_main, exchangeusd, price_vi, price_en, amount, total_vi, total_en, note, userid_edit, update_date, adminid, crt_date, weight, active) VALUES (:debitnoteid, :pid, :cid, :yearmonth, :adddate, :serviceid, :service_name, :service_main, :exchangeusd, :price_vi, :price_en, :amount, :total_vi, :total_en, :note, :userid_edit, ' . NV_CURRENTTIME . ', :adminid, ' . NV_CURRENTTIME . ', :weight, :active)');
									...
									$stmt->bindParam(':adddate', $row['debitnotedate'], PDO::PARAM_INT);
									$stmt->bindParam(':serviceid', $value['sid'], PDO::PARAM_INT);
									$stmt->bindParam(':service_name', $row['title'], PDO::PARAM_STR);
									$stmt->bindParam(':exchangeusd', $row['exchangeusd'], PDO::PARAM_INT);
									$stmt->bindParam(':price_vi', $value['pricevnd'], PDO::PARAM_STR);
									$stmt->bindParam(':price_en', $value['priceusd'], PDO::PARAM_STR);
									$stmt->bindParam(':amount', $value['amount'], PDO::PARAM_STR);
